...
HANDLE saveVars(const struct var_info_t* local_var_info)
{
	...
	hMapping = CreateFileMapping(INVALID_HANDLE_VALUE, &securityAttributes, PAGE_READWRITE, 0, sizeOfVars(global_var_info) + sizeOfVars(local_var_info) + file_list_size(first_flist) + file_list_size(cur_flist) + file_list_size(dir_flist), NULL);
	...
	if (DEBUG_GTE(MINGW, 3)) rprintf(FINFO, "copying variables to shared memory\n");
	...
	if (DEBUG_GTE(MINGW, 3)) rprintf(FINFO, "copying file list\n");
  
	save_file_list(sharedVar, the_file_list);

	UnmapViewOfFile(shared);

	if (DEBUG_GTE(MINGW, 3)) rprintf(FINFO, "variables copied\n");
...
void restoreVars(HANDLE hMapping, const struct var_info_t* local_var_info)
{
	...
	if (DEBUG_GTE(MINGW, 3)) rprintf(FINFO, "restoring variables from shared memory\n");
	...
	if (DEBUG_GTE(MINGW, 3)) rprintf(FINFO, "restoring file list\n");
	...
	*((struct file_list**) local_var_info[2].var) = the_file_list;
	...
	if (DEBUG_GTE(MINGW, 3)) rprintf(FINFO, "variables restored\n");
}
...
size_t file_list_size(struct file_list* flist)
{
	...
	for (list = flist; list = list->next; !list)
	{
